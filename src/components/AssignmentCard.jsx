...
const AssignmentCard = ({ category,content, title, priority, avatars, extraCount }) => {
    return (
            ...
            <div className='rounded-xl border group-hover:cursor-pointer group-hover:shadow-lg group-hover:shadow-gray-300'>
                <div className='space-y-3 text-sm px-3 pt-2'>
                    ...
                    <div className='flex bg-[#EBF6FF] w-fit py-[9px] px-5 rounded-lg items-center space-x-2'>
                        ...
                        <p className='text-[#03A1D8] uppercase text-[12px] '>{category}</p>
                    </div>
                    <div className=' '>
                        <p className={`uppercase py-2 px-5 rounded-lg text-[12px] font-semibold ${priority === 'HIGH' ? 'bg-[#FBE7E9] text-[#D84848]' : 'bg-[#FEF0C7] text-[#F8A029]'}`}>{priority}</p>
                    </div>
                    </div>
